...
void loop() {
  ...
  if (isnan(h) || isnan(t) || isnan(f)) {
    ...
    // выводим на дисплей
    display.clear();
    display.drawString(0, 0, "Humidity: " + String(h) + "%\t");  // "Влажность: "
    display.drawString(0, 16, "Temp: " + String(t) + "C");  // "Температура: "
    display.drawString(0, 32, "Temp: " + String(f) + "F");  // "Температура: "
